...
export default function Navbar() {
    ...
    return (
        <header
            className={`fixed left-0 right-0 z-50 flex justify-center px-4 w-full transition-all duration-500 ease-in-out ${isScrolled ? "top-4" : "top-8"
                }`}
        >
            <div
                ref={navRef}
                className={`transition-all duration-500 ease-out flex items-center justify-between w-full shadow-2xl ${isScrolled
                    ? "max-w-4xl glass-nav-compact py-1.5 px-1.5 rounded-[2rem] border border-white/10"
                    : "max-w-5xl glass-nav py-2 px-2 rounded-full border border-white/5"
                    }`}
            >
                <div className={`transition-all duration-500 flex items-center ${isScrolled ? "pl-5" : "pl-6"}`}>
                    <a className="hover:opacity-80 transition-opacity cursor-pointer flex items-center">
                        <svg id="uuid-10cf10d3-5297-4b86-b70b-54e50878cb27" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 848.3 251.27" className={`transition-all duration-500 w-auto fill-white ${isScrolled ? "h-6" : "h-8"}`}>
                ...
                <nav className="hidden md:flex items-center gap-1 mx-4">
                    {siteContent.navbar.links.map((link: { label: string, href: string }, i: number) => (
                        <a
                            key={i}
                            href={link.href}
                            className={`px-4 py-2 text-white hover:bg-white/10 rounded-full text-sm font-medium transition-all cursor-pointer ${isScrolled ? "text-[13px]" : "text-sm"
                                }`}
                        >
                            {link.label}
                        </a>
                ...
                <div className="pr-1.5">
                    <a
                        href={siteContent.navbar.button.href}
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`bg-coral-gradient rounded-full text-white font-medium hover:opacity-90 transition-all shadow-[0_4px_14px_rgba(255,107,107,0.4)] cursor-pointer inline-block text-center ${isScrolled ? "px-5 py-2 text-xs" : "px-6 py-2.5 text-sm"
                            }`}
                    >
                        {siteContent.navbar.button.label}
                    </a>
                </div>
            </div>
        </header>
