            ...
            <div className="w-full max-w-7xl mx-auto flex flex-col gap-12 lg:gap-16 relative z-10">
                ...
                <section ref={listRef} className="flex flex-col gap-4 w-full">
                    {siteContent.servicos.items.map((item, index) => {
                        ...
                        if (isActive) {
                            return (
                                <article key={index} className="bg-[#141419]/60 border border-white/10 relative backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden rounded-[24px] p-8 md:p-10 w-full transition-all duration-500 group">
                                    ...
                                    <div className="flex flex-col lg:flex-row gap-8 relative z-10">
                                        <div className="flex-shrink-0 flex flex-col justify-between lg:w-1/3 min-h-[300px] lg:min-h-[400px]">
                                            <div className="flex flex-col gap-8">
                                                <span className="font-display text-7xl md:text-8xl font-light text-gradient-coral leading-none opacity-90">{item.number}</span>
                                                <div className="mt-2">
                                                    <h2 className="font-display text-4xl md:text-5xl font-light text-white mb-6">{item.title}</h2>
                                        ...
                                        <div className="flex-grow flex flex-col md:flex-row gap-8 items-center lg:items-stretch group/img">
                                            <div className="relative w-full md:w-1/2 aspect-[4/3] md:aspect-auto rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 border border-white/5">
                                                {/* Fallback to branding image if not specified */}
                                                <img alt={item.title as string} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover/img:opacity-100 group-hover/img:scale-105 transition-all duration-700 grayscale-[20%] hover:grayscale-0" src={item.image || "/images/services-branding.jpg"} />
                        ...
                        return (
                            <article key={index} onClick={() => setActiveIndex(index)} className="bg-white/[0.02] border border-white/[0.06] backdrop-blur-[12px] shadow-[0_4px_30px_rgba(0,0,0,0.3)] rounded-[24px] p-6 md:p-8 w-full hover:bg-white/[0.03] transition-all duration-300 cursor-pointer group">
                                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative">
                                    <div className="flex items-center gap-8 md:gap-16 w-full md:w-auto">
                                        <span className="font-display text-5xl md:text-7xl font-light text-white/10 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-br group-hover:from-white/40 group-hover:to-white/10 transition-all duration-500">{item.number}</span>
                                        <div className="flex flex-col gap-3">
                                            <h3 className="font-display text-2xl md:text-3xl font-light text-white/80 group-hover:text-white transition-colors tracking-wide">{item.title}</h3>
